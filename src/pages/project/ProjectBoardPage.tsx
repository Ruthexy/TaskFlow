...
export function ProjectBoardPage() {
  ...
  const handleTaskClick = (task: Task) => { setSelectedTask(task); setTaskModalOpen(true) }
  const handleAddTask = (colId: string) => { setSelectedTask(null); setDefaultColumnId(colId); setTaskModalOpen(true) }
      ...
      <div className="flex-1 overflow-x-auto">
        ...
        >
          <div className="flex gap-4 p-6 min-w-max h-full">
            {columns.map((col) => (
              <KanbanColumn key={col.id} column={col} onTaskClick={handleTaskClick} onAddTask={handleAddTask} />
            ))}
          </div>
          <DragOverlay>
